...
from country
where IndepYear in (800,1810,1901) 
...
-- 11. '홍길동'과 'hong'의 글자 길이를 각각 출력하시오. 왜 한글은 length 일경우 byte로 계산 1글자: 3byte
...
select name, insert(name, 3, length(name)-4, repeat('*', length(name)-4)) as guess
from country; 

-- 15. country에서 region을 중복 없이 가져오는데 공백을 _로 대체하시오. region의 길이로 정렬한다.(25건)
select distinct replace(region, ' ', '_') as region 
from country
order by length(region) desc;

-- 16. country에서 인구가 1억 이상인 국가들의 1인당 점유면적(surfacearea/population)을 반올림해서 소숫점 3자리로 표현하시오.
--  1인당 점유 면적의 오름차순으로 정렬한다.(10건)
...
select curdate() 오늘, dayofyear(curdate()) as "올해 지난 날", date_add(curdate(), interval 100 day) as "100일 후"
from dual;

-- 18. country에서 asia에 있는 나라 중 희망 수명이 있는 경우에 기대 수명이 80 초과면 장수국가, 60 초과면 일반국가, 
-- 그렇지 않으면 단명국가라고 표현하시오. 기대 수명으로 정렬한다.(51건)
...
select weekday('2020-05-05'), 
	case when weekday('2020-05-05') < 5 then '행복'
		else '불행'
	end "행복여부"
from dual;

-- 20-2. 2021년 어린이 날이 평일이면 행복, 토요일 또는 일요일이면 불행이라고 출력하시오.
select weekday('2021-05-05'), 
	case when weekday('2020-05-05') < 5 then '행복'
		else '불행'
	end "행복여부"
from dual;