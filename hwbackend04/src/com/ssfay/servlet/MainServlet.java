...
public class MainServlet extends HttpServlet {
	...
	protected void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ("register".equals(act)) {
			...
			request.getRequestDispatcher(path).forward(request, response);
		} else if ("searchName".equals(act)) {
			path=searchProductbyName(request, response);
			request.getRequestDispatcher(path).forward(request, response);
		} else if ("searchPrice".equals(act)) {
			path=searchProductbyPrice(request, response);
			request.getRequestDispatcher(path).forward(request, response);
		}
		
	}
	
	private String searchProductbyPrice(HttpServletRequest request, HttpServletResponse response) {
	...
	private String searchProductbyName(HttpServletRequest request, HttpServletResponse response) {
		HttpSession session=request.getSession();
		UserDto userDto=(UserDto) session.getAttribute("userinfo");
		if (userDto!=null) {
